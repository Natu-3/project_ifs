                ...
                canvas.toBlob(
                    (blob) => {
                        ...
                        console.log('이미지 리사이즈 완료:', {
                            ...
                            resizedSizeMb: `${(resizedFile.size / 1024 / 1024).toFixed(2)}MB`,
                            originalResolution: `${img.width}x${img.height}`,
                            resizedResolution: `${Math.round(width)}x${Math.round(height)}`,
                        });
...
export const extractTextFromImage = async (imageFile, fields = [], format = 'json') => {
    try {
        const requestedFields = Array.isArray(fields) && fields.length > 0 ? fields : ['text'];
        ...
        params.append('fields', JSON.stringify(requestedFields));
        ...
        if (error.response) {
            ...
            const requestDataPreview =
                typeof requestData === 'string'
                    ...
                    : requestData instanceof FormData
                        ? '[FormData payload]'
                        : '[No string preview available]';

            console.error('Response headers:', error.response.headers);
            console.error('Request url:', error.config?.url);
            console.error('Request headers:', error.config?.headers);
            console.error('Request data length:', requestDataLength);
            console.error('Request data preview:', requestDataPreview);
            console.error('API key prefix:', `${error.config?.headers?.['x-rapidapi-key']?.substring(0, 20) || ''}...`);
            console.error('Check browser network tab for full upstream API response.');
